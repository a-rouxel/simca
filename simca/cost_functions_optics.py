...
# def get_catalog(catalog, device="cpu"):
#     hoya_pd = get_glass_catalog(catalog)

#     list_of_glasses = hoya_pd.df.iloc[:, 0].index.tolist()
#     idx = pd.IndexSlice
#     nd_values = hoya_pd.df.loc[:, idx[pd.NA, 'nd']]
#     vd_values = hoya_pd.df.loc[:, idx["abbe number", 'vd']]
#     nd_values = torch.tensor(nd_values.values, device=device)
#     vd_values = torch.tensor(vd_values.values, device=device)
...
def plot_grids_coordinates(cassi_system,test_name='test',dir_name=None):
    ...
    if dir_name is not None:
        plt.savefig(dir_name +f"grids_coordinates_{test_name}.svg")
    
    plt.show()

    if dir_name is not None:
        np.save(dir_name + f"x_coordinates_{test_name}",x_vec_no_dist.detach().cpu().numpy())
        np.save(dir_name + f"y_coordinates_{test_name}",y_vec_no_dist.detach().cpu().numpy())
        np.save(dir_name + f"wavelengths_{test_name}",cassi_system.wavelengths.detach().cpu().numpy())
        print(cassi_system.wavelengths.detach().cpu().numpy())
...
def initialize_optimizer(cassi_system, params_to_optimize):
    ...
    param_groups = [
        {'params': base_params, 'lr': 0.001},  # Base learning rate
        {'params': glass_params, 'lr': 0.01}   # Higher learning rate for nd1 and vd1
    ]
...
def evaluate_optical_performances(cassi_system,dir_name=None):
    ...
    distortion_metric, _,mean_distortion = evaluate_distortions(x_vec_out, y_vec_out, x_vec_no_dist, y_vec_no_dist,dir_name)
...
def evaluate_distortions(X_vec_out_distor, Y_vec_out_distors, X_vec_out, Y_vec_out,dir_name=None):
    ...
    if dir_name is not None:
        ...
        for i in [0,5,-1]:
            ...
            plt.colorbar()
            plt.savefig(dir_name + f"distor_map_{i}.svg")
            # plt.show()
...
def optimize_cassi_system(params_to_optimize,target_dispersion, cost_weights, config_path, iterations, patience, results_dir,index_estimation_method="cauchy",device='cpu'):
    config_system = load_yaml_config(config_path)
    cassi_system = CassiSystemOptim(system_config=config_system,device=device,index_estimation_method=index_estimation_method)
...
def test_cassi_system(config_path,device,index_estimation_method="sellmeier"):

    config_system = load_yaml_config(config_path)
    cassi_system = CassiSystemOptim(system_config=config_system,index_estimation_method=index_estimation_method,device=device)
