...
class Dimensionner():
    ...
    def __init__(self,config):
        ...
        self.n_array = np.full(self.X_cam.shape, sellmeier(self.config["spectral range"]["wavelength center"]))
    ...
    def retropropagate(self):
        ...
        if self.config["system architecture"]["dispersive_element_1_type"] == "grating":
            ...
            self.X_cam_flatten = self.X_cam.flatten()
            self.Y_cam_flatten = self.Y_cam.flatten()
            self.n_array_flatten = self.n_array.flatten()
            ...
            X_dmd, Y_dmd = find_cam_position_curve_fit_new_model_no_A_oneinput(self.X_cam_flatten , self.Y_cam_flatten,self.n_array_flatten, self.config["system architecture"]["dispersive_element_1_A"], self.config["system architecture"]["focal_lens_1"], self.alpha_c, self.config["system architecture"]["dispersive_element_1_delta_alpha_c"], self.config["system architecture"]["dispersive_element_1_delta_beta_c"])

            X_dmd = X_dmd.reshape((self.X_cam.shape))
            Y_dmd = Y_dmd.reshape((self.Y_cam.shape))
            return [X_dmd, Y_dmd]
